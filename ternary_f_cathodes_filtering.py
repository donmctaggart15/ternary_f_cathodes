...
mpr = MPRester("eXE8IJUp6lZSxLAo")             # Legacy MP API key, must use this for interface reactions


#%%   MAJOR SECTIONS OF CODE

#   1: Filtering MatProj database via Simmate 
...
#%% Create all lists: CAREFUL! WILL ERASE PREVIOUS DATA 
...
def FTransfer(m1,m2,scale_factor):
    ...
    if scale_factor != 1:       
        ...
        return m1.atoms_per_fu - m2.atoms_per_fu   # This line only was used previously
...
# Voltage vs F2    
def VoltageF2(m1_f, m2_def, e_per_a_f, e_per_a_def, f_transfer, scale_factor):
    # -3.2684 is F2 energy
    if scale_factor != 1:
        return ((m1_f.atoms_per_fu * e_per_a_f) - ((m2_def.atoms_per_fu / scale_factor)* e_per_a_def + (f_transfer/2) * -3.2684))  / -f_transfer
    else:
        return ((m1_f.atoms_per_fu * e_per_a_f) - (m2_def.atoms_per_fu* e_per_a_def + (f_transfer/2) * -3.2684))  / -f_transfer           
...
#%% 1.1: FILTER MATPROJ DATABASE TO INCLUDE ALL F STRUCTURES WITH MORE THAN 2 ELEMENTS WITHIN 75meV OF HULL
...
#%% 1.2: Make list of Structure objects from ids so that F can be removed
...
print("Len of strucs_list_complete:", len(strucs_list_complete))
...
for i,struc in enumerate(strucs_list_complete):
    ...
    if len(filter_possible_df) > 0:
        print("Match for", strucs_df_complete.formula_reduced[i], ":", filter_possible_df.formula_reduced[0])
        ...
        bandgap_def = strucs_df_complete.band_gap[0]
        ...
        print("Zero matching for index:", i)
...
#%% 3.1 PARTIAL DEF PAIRS FILTER 

print("Len of strucs_list_partial:", len(strucs_list_partial))
...
for i,struc in enumerate(strucs_list_partial):
    ...
    if len(filter_possible_df) > 0:
        print("Possible match (same elements) for index:", i)
        ...
        for k,m in enumerate(filter_possible_list):
            ...
            if struc_def_element1 == possible_def_element1 and struc_def_element2 == possible_def_element2 and struc_def_element3 == possible_def_element3 and struc_elementF > possible_elementF:                   
                print("A REAL MATCH, same elements and ratio:", struc[0].composition.reduced_formula, "-->", m[0].composition.reduced_formula)               
                ...
                bandgap2_def = strucs_df_partial.band_gap[k]
        ...
        print("No possible matches for index:", i)           
                

#%% 3.2: Copy lists so that partial filter process doesnt have to run again
...
for i,x in enumerate(materials_list_f):
    ...
    voltagef2_list.append(VoltageF2(x, y, energy_per_atom_list_f[i], energy_per_atom_list_def[i], f_transfer_list[i], sf))
...
#%% Calculate energy density for LiCoO2
...
#li_voltage  = 3.7 # conventional voltage
...
li_scale = ScaleFactorDeFbyF(li_mat,coo2_mat)

volume_per_li = VolumePerF(li_mat, 1) # 1 Li per fu

li_voltage = ((4*li_epa[0] + 6*carb_epa[0]) - (3*coo2_epa[0] + 7*carb_li_epa[0])) / -1        

li_grav_cap = GravCapacity(1, li_mat.molar_mass)

li_grav_eden2 = GravEnDen(li_grav_cap, li_voltage)
li_vol_eden2 = VolEnDen(li_grav_eden2, li_mat.density)
...
for i,x in enumerate(composition_list_f):
    ...
    #log_cost_p_mol_p_f_f = math.log(abs(cost_per_mol_per_f_f))
...
complete_def_pairs_table = pd.DataFrame({
    ...
    'voltage_lif': voltage_list,
    'voltage_f2': voltagef2_list,
...
complete_def_pairs_table.to_csv('complete_cathode_list.csv')
